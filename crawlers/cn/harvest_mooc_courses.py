"""按技能名采集中国大学MOOC 的课程，替换检索落地页。

⚠ 2026-08-18 起停用，不要再跑
-----------------------------
产出的课程**不满足「能真实学习」的口径**：要登录报名才能看内容，且按学期开课，
往期课程结束后非选课用户点进去只剩介绍页。库里那 129 门已 `archived`
（`scripts/archive_courses.py`），读路径把这类判为 `kind=enroll` 而非 `real`。
`scripts/run_full_chain.py` 已把课程步骤移出默认流程（要 `--with-courses`）。

脚本保留是因为将来若接入「跳转报名」形态还用得上 —— 那时它是合格的 enroll 源。
现在跑一次就会把报名墙课程重新写回 SQLite，重灌时复活。
